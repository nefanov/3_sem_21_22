...
void Read(int fd, void *buf, size_t nbytes) {
  ssize_t noread = read(fd, buf, nbytes);
  if (noread == -1) {
    perror("read error");
    exit(1);
  }
...
int get_start(int fd_server, struct easy_message *msg) {
  ...
  create_Fifo_name(msg->sm_clientpid, fifo_name, sizeof(fifo_name));
  printf("fifo_name: %s\n", fifo_name);

  struct easy_message ready_msg;
...
int main() {
  ...
  // Make connection
  struct easy_message msg;
  int fd_client = get_start(fd_server, &msg);
